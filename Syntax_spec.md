...
C<< supports only single-line comments:
...
### 10.6 Anonymous Blocks (Sub-arenas)
...
```
bool       case       char       const      default    def
else       enum       entry      false      float32    float64
for        foreach    if         import     int8       int16
int32      int64      move       namespace  raw        reserve
reset      string     struct     switch     true       tunnel
uint8      uint16     uint32     uint64     valid      voided
while
```

-----

## 20. Complete Operator Table
