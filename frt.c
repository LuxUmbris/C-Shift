...
void *__cshift_arena_realloc_track(cshift_arena_t *a, void *old_ptr, void *new_ptr);
...
{
  char *s = read_line();
  if (arena && s) __cshift_arena_push(arena, s);
  return s;
}
...
char *read_file_s(const char *path)
{
  ...
  long sz = ftell(f);
  rewind(f);
  if (sz < 0 || (unsigned long)sz > (1UL << 30)) { fclose(f); return NULL; } /* 1 GB limit */
...
char *str_to_upper_a(const char *s, cshift_arena_t *arena)
{
  ...
  if (arena) __cshift_arena_push(arena, r);
  return r;
}
...
char *str_repeat_a(const char *s, int n, cshift_arena_t *arena)
{
  ...
  if (arena && r) __cshift_arena_push(arena, r);
  return r;
}
...
char **str_split_a(const char *s, const char *delim, int *out_count, cshift_arena_t *arena)
{
  ...
  if (dlen == 0) { *out_count = 0; return NULL; }
  int count = 1;
  const char *p = s;
  ...
  if (arena) __cshift_arena_push(arena, parts);
  ...
  while ((found = strstr(p, delim)) != NULL)
  {
    ...
    memcpy(part, p, len2);
    part[len2] = '\0';
    if (arena) __cshift_arena_push(arena, part);
    parts[i++] = part;
    p = found + dlen;
  }
  /* last part */
  parts[i] = strdup(p);
  if (arena && parts[i]) __cshift_arena_push(arena, parts[i]);
  *out_count = count;
  return parts;
}
...
/* For buffers that grow via realloc() (e.g. T[] arena arrays): instead of
 * pushing a brand-new tracked entry on every grow (which would leave stale
 * pointers in the list that realloc() may have already freed, causing a
 * double-free at scope exit), this updates the LAST tracked entry in place
 * to point at the new address. The very first allocation for a given
 * buffer must still be registered with __cshift_arena_push (or this
 * function, if old_ptr is NULL/not found — it falls back to pushing).        */
void *__cshift_arena_realloc_track(cshift_arena_t *a, void *old_ptr, void *new_ptr)
{
    if (!new_ptr) return new_ptr;
    /* Search from the end (most recent) for old_ptr and replace it in place. */
    ...
    /* Not previously tracked (first allocation for this buffer) — push it. */
    return __cshift_arena_push(a, new_ptr);
}
...
void *vec_new(size_t initial_cap)
{
    /* Elements are always stored as 8-byte (uint64) words — type erasure.
     * The C<< codegen coerces all element types (int8..int64, float32/64,
     * pointers) to a 64-bit word before push/get, so a fixed 8-byte stride
     * is always correct regardless of the declared Vector<T> element type.
     * `initial_cap` is a capacity hint (number of elements to
     * pre-allocate) — NOT a byte size. Previously this parameter was
     * mis-used as elem_sz, causing massive over-allocation (e.g.
     * vec_new(1024) would allocate 1024 BYTES per element instead of
     * reserving 1024 elements) and a buffer overflow once push() wrote
     * only 8 bytes per slot into a stride that codegen elsewhere assumed
     * was narrower. */
    CShiftVec *v = (CShiftVec *)calloc(1, sizeof(CShiftVec));
    if (!v) return NULL;
    v->elem_sz = 8; /* always 8-byte word storage */
    if (initial_cap > 0)
    {
        v->data = malloc(initial_cap * 8);
        if (v->data) v->cap = initial_cap;
    }
    return v;
}
...
int vec_pop(CShiftVec *v, void *out)
{
    ...
    v->len--;
    if (out) memcpy(out, (char *)v->data + v->len * v->elem_sz, v->elem_sz);
    return 0;
}
...
{
    if (!v || idx >= v->len) return -1;
    if (out) memcpy(out, (char *)v->data + idx * v->elem_sz, v->elem_sz);
    size_t tail = (v->len - idx - 1) * v->elem_sz;
    if (tail > 0)
        memmove((char *)v->data + idx * v->elem_sz,
                (char *)v->data + (idx + 1) * v->elem_sz, tail);
    v->len--;
    return 0;
}
...
void vec_clear(CShiftVec *v) { if (v) v->len = 0; }
...
int sb_append(CShiftSB *sb, const char *s)
{
    ...
    if (sb_ensure(sb, slen) < 0) return -1;
...
void sb_clear(CShiftSB *sb) { if (sb) sb->len = 0; if (sb && sb->buf) sb->buf[0] = '\0'; }
void sb_free(CShiftSB *sb) { if (!sb) return; free(sb->buf); free(sb); }
...
{
    if (!m || !key) return -1;
    if (m->used * 2 >= m->cap && map_grow(m) < 0) return -1;
    size_t i = map_hash(key, m->cap);
    while (m->keys[i] && strcmp(m->keys[i], key) != 0)
        i = (i + 1) % m->cap;
    if (!m->keys[i]) { m->keys[i] = strdup(key); m->used++; }
    m->vals[i] = val;
    return 0;
}
...
{
    if (!m || !key || !m->cap) return 0;
    size_t i = map_hash(key, m->cap);
    while (m->keys[i])
    {
        if (strcmp(m->keys[i], key) == 0)
        {
            if (out) memcpy(out, &m->vals[i], 8);
            return 1;
        }
        i = (i + 1) % m->cap;
    }
...
int map_remove(CShiftMap *m, const char *key)
{
    ...
    while (m->keys[i])
    {
        if (strcmp(m->keys[i], key) == 0)
        {
            free(m->keys[i]); m->keys[i] = NULL; m->vals[i] = 0; m->used--;
            return 1;
        }
        i = (i + 1) % m->cap;
    }
...
{
    if (!m) return;
    for (size_t i = 0; i < m->cap; i++) { free(m->keys[i]); m->keys[i] = NULL; m->vals[i] = 0; }
    m->used = 0;
}
void map_free(CShiftMap *m) { if (!m) return; map_clear(m); free(m->keys); free(m->vals); free(m); }

/* ── LinkedList<T> ─────────────────────────────────────────────────────────── */
typedef struct CShiftListNode { struct CShiftListNode *next, *prev; unsigned long long data; } CShiftListNode;
typedef struct { CShiftListNode *head, *tail; size_t len; size_t elem_sz; } CShiftList;
...
unsigned long long list_get(CShiftList *l, unsigned long long idx)
{
    if (!l) return 0;
    CShiftListNode *nd = l->head;
    for (unsigned long long i = 0; i < idx && nd; i++) nd = nd->next;
    return nd ? nd->data : 0;
}
...
void list_clear(CShiftList *l)
{
    if (!l) return;
    CShiftListNode *nd = l->head;
    while (nd) { CShiftListNode *nx = nd->next; free(nd); nd = nx; }
    l->head = l->tail = NULL; l->len = 0;
}
void list_free(CShiftList *l) { if (!l) return; list_clear(l); free(l); }

/* ── Set<T> (sorted array) ─────────────────────────────────────────────────── */
typedef struct { unsigned long long *data; size_t len, cap; } CShiftSet;
...
int set_insert(CShiftSet *s, unsigned long long v)
{
    ...
    while (lo < hi) { size_t mid = (lo+hi)/2; if (s->data[mid] < v) lo=mid+1; else hi=mid; }
    if (lo < s->len && s->data[lo] == v) return 0; /* already present */
    if (s->len >= s->cap) {
...
int set_remove(CShiftSet *s, unsigned long long v)
{
    ...
    size_t lo=0, hi=s->len;
    while (lo<hi) { size_t mid=(lo+hi)/2; if (s->data[mid]<v) lo=mid+1; else hi=mid; }
    if (lo>=s->len || s->data[lo]!=v) return 0;
...
void set_free(CShiftSet *s) { if (!s) return; free(s->data); free(s); }

/* ── BitSet ─────────────────────────────────────────────────────────────────── */
typedef struct { unsigned char *data; size_t nbits; } CShiftBitSet;
...
{
    if (!bs || i>=bs->nbits) return -1;
    bs->data[i/8] |= (1<<(i%8)); return 0;
}
...
{ if(!d||!s) return; size_t nb=(d->nbits<s->nbits?d->nbits:s->nbits+7)/8; for(size_t i=0;i<nb;i++) d->data[i]&=s->data[i]; }
void bitset_or(CShiftBitSet *d, CShiftBitSet *s)
{ if(!d||!s) return; size_t nb=(d->nbits<s->nbits?d->nbits:s->nbits+7)/8; for(size_t i=0;i<nb;i++) d->data[i]|=s->data[i]; }
void bitset_xor(CShiftBitSet *d, CShiftBitSet *s)
{ if(!d||!s) return; size_t nb=(d->nbits<s->nbits?d->nbits:s->nbits+7)/8; for(size_t i=0;i<nb;i++) d->data[i]^=s->data[i]; }
void bitset_not(CShiftBitSet *bs)
{ if(!bs) return; size_t nb=(bs->nbits+7)/8; for(size_t i=0;i<nb;i++) bs->data[i]=~bs->data[i]; }
void bitset_free(CShiftBitSet *bs) { if(!bs) return; free(bs->data); free(bs); }
...
int svec_push(CShiftSVec *sv, unsigned long long v)
{
    ...
    if (sv->len >= sv->cap) {
        ...
        sv->data = realloc(sv->data, nc*sv->elem_sz); sv->cap = nc;
    }
    /* insertion sort */
    size_t i = sv->len;
    while (i > 0) {
        unsigned long long prev; memcpy(&prev, (char*)sv->data+(i-1)*sv->elem_sz, sv->elem_sz);
        if (prev <= v) break;
        memcpy((char*)sv->data+i*sv->elem_sz, &prev, sv->elem_sz); i--;
    }
...
{
    if (!sv || idx >= sv->len) return 0;
    unsigned long long w=0; memcpy(&w, (char*)sv->data+idx*sv->elem_sz, sv->elem_sz); return w;
}
...
int svec_find(CShiftSVec *sv, unsigned long long v)
{
    ...
    while(lo<hi){size_t mid=(lo+hi)/2; unsigned long long w=0; memcpy(&w,(char*)sv->data+mid*sv->elem_sz,sv->elem_sz); if(w<v)lo=mid+1; else hi=mid;}
    if(lo<sv->len){unsigned long long w=0;memcpy(&w,(char*)sv->data+lo*sv->elem_sz,sv->elem_sz);if(w==v)return(int)lo;}
    return -1;
}
int svec_remove(CShiftSVec *sv, unsigned long long v)
{
    if (!sv) return -1;
    int idx = svec_find(sv, v); if (idx<0) return -1;
    memmove((char*)sv->data+idx*sv->elem_sz,(char*)sv->data+(idx+1)*sv->elem_sz,(sv->len-idx-1)*sv->elem_sz);
    sv->len--; return 0;
}
void svec_free(CShiftSVec *sv) { if(!sv) return; free(sv->data); free(sv); }
...
int map_contains(CShiftMap *m, const char *key)
{ return map_has(m, key); }


#ifdef CSHIFT_ARENA_DEBUG
#include <stdio.h>
void __cshift_arena_debug_dump(cshift_arena_t *a)
{
    fprintf(stderr, "[arena] count=%zu cap=%zu\n", a->count, a->cap);
