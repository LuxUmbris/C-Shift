...
private:
    ...
    Lexer::Token match_token(Lexer::TokenType expected, const std::string& expected_value = "")
    {
        ...
        throw std::runtime_error(
            ...
            "', expected: '" + (expected_value.empty() ? "<token>" : expected_value) + "'");
    }

    size_t current_line()
    {
        return peek_token().line;
    }
    ...
    ASTNode* parse_top_level()
    {
        if (peek_token().type == Lexer::TokenType::KEYWORD)
        {
            ...
            if (kw == "def")       return parse_function();
            if (kw == "enum")      return parse_enum();
            if (kw == "tunnel")    return parse_tunnel();
            if (kw == "reserve")   return parse_reserve();
            if (kw == "move")      return parse_move();
            if (kw == "namespace") return parse_namespace();
            if (kw == "const")     return parse_const();
            if (kw == "reset")     return parse_reset();
            if (kw == "entry") {
                if (has_entry) throw std::runtime_error("[SYNTAX ERROR] Multiple 'entry' definitions found");
                has_entry = true;
                return parse_entry();
            }
            if (is_type_keyword(kw)) return parse_declaration();
            throw std::runtime_error("[SYNTAX ERROR] Line " + std::to_string(current_line()) + ": Unexpected keyword: " + kw);
        }
    ...
    bool is_type_keyword(const std::string& kw)
    {
        ...
        for (auto& t : type_kws) if (t == kw) return true;
        return false;
    }
    ...
    ASTNode* parse_parameters()
    {
        ...
        while (peek_token().value != ")" && peek_token().type != Lexer::TokenType::END_OF_FILE)
        {
            ...
            if (peek_token().value == ",") advance_token();
        }
        match_token(Lexer::TokenType::OPERATOR, ")");
        return params;
    }

    // Parses a type including pointer/slice suffixes: int32, int32*, int32[], int32[:]
    std::string parse_type_string()
    {
        ...
        {
            throw std::runtime_error("[SYNTAX ERROR] Line " + std::to_string(current_line()) + ": Expected type, got: " + peek_token().value);
        }
        // pointer
        while (peek_token().value == "*") { type += "*"; advance_token(); }
        // slice or array
        if (peek_token().value == "[")
        {
            advance_token();
    ...
    ASTNode* parse_expression()
    {
        ...
        while (peek_token().type != Lexer::TokenType::END_OF_FILE)
        {
            ...
            if (paren_depth == 0 && (val == ";" || val == "}" || val == "]" || val == "->"))
                break;
    ...
    ASTNode* parse_tunnel()
    {
        ...
        node->children.push_back(new ASTNode("Target", target_type + " " + target_name, ln, current_depth));
    ...
    ASTNode* parse_struct()
    {
        ...
        while (peek_token().value != "}" && peek_token().type != Lexer::TokenType::END_OF_FILE)
        {
            ...
            match_token(Lexer::TokenType::OPERATOR, ";");
            struct_node->children.push_back(new ASTNode("Field", type + " " + field_name, ln, current_depth));
        }
        match_token(Lexer::TokenType::OPERATOR, "}");
        return struct_node;
    }
    ...
    ASTNode* parse_enum()
    {
        ...
        while (peek_token().value != "}" && peek_token().type != Lexer::TokenType::END_OF_FILE)
        {
            ...
            if (!explicit_val.empty()) ev->value += "=" + explicit_val;
    ...
    ASTNode* parse_reserve()
    {
        ...
        {
            advance_token();
            if (peek_token().value == "shared") { advance_token(); is_shared = true; }
            match_token(Lexer::TokenType::OPERATOR, ">");
        }
        ...
        if (is_shared) node->children.push_back(new ASTNode("Shared", "shared", ln, current_depth));
