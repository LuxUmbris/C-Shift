 ...
 *   linker      Preferred linker binary name (e.g. "ld.lld",
 * "aarch64-linux-gnu-ld") Set to "" to auto-detect. sysroot     Path to target
 * sysroot for cross-linking. Set to "" for native or bare-metal (no libc).
 *   dynamic_linker
...
static inline EzTarget *ez_target_arm32_linux(void)
{
  ...
  ez__fill_target(t, "armv7-linux-gnueabihf", "generic", "+vfp3,+neon",
                  "ld.lld", "/usr/arm-linux-gnueabihf",
 ...
 */
static inline void ez_dump(EzModule *m) { LLVMDumpModule(m->mod); }
 ...
 * @param nobj        Number of object files.
 * @param extra_flags Extra flags (linker scripts, -T, --entry, etc.). May be
 * NULL.
 ...
 *   EzTarget *rv = ez_target_bare_metal("riscv32-unknown-elf", "generic-rv32",
 * "+m,+a"); const char *flags[] = { "-T", "flash.ld", "--entry",
 * "reset_handler" }; ez_link_exe_for(rv, objs, 1, flags, 4, "firmware.elf");
 */
 ...
 *   ez_target_free(arm);
 *
 * @param m          Module (should have been created with
 * ez_module_for_target).
 * @param tgt        Target.
 * @param out_path   Output executable path.
                              ...
                              EzType *params, unsigned nparams, int vararg)
{
  ...
  assert(f);
  f->owner = m;
  f->fn = LLVMAddFunction(m->mod, name,
                          LLVMFunctionType(ret, params, nparams, vararg));
  return f;
}
