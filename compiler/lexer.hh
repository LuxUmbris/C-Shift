...
public:
    ...
    {
        IDENTIFIER, KEYWORD, NUMBER, STRING, OPERATOR, END_OF_FILE
    };
        ...
        while (pos < src.length()) 
        {
            ...
            for (const auto& [op, type] : operator_lookup) 
            {
                if (src.compare(pos, op.length(), op) == 0) 
                {
                    ...
                    for(size_t i = 0; i < op.length(); ++i) advance();
                    found_op = true;
                    break;
                }
            }
...
private:
    Token readString() {
        ...
        while (peek() != '"' && peek() != '\0') 
        {
            if (peek() == '\\') val += advance(); // Basic escape pass-through
            val += advance();
        }
        advance(); // Skip "
    ...
    Token readRawString() {
        for(int i = 0; i < 4; ++i) advance(); // Skip "raw<"
        std::string content;

        if (src.compare(pos, 5, "until") == 0) 
        {
            // Case: raw<until "DELIM">
            for(int i = 0; i < 6; ++i) advance(); // Skip "until "
            ...
            // Case: raw<N> (N lines)
            std::string numStr;
            while(std::isdigit(peek())) numStr += advance();
            advance(); // Skip ">"
            if(peek() == '\n') advance();
    ...
    Token readIdentifierOrKeyword() 
    {
        ...
        if (keyword_lookup.count(val)) return {keyword_lookup[val], val, line};
